...
let SCONFIG_URL = `https://${process.env.SCONFIG_GATEWAY || 'api.sconfig.io'}/config`;
const THORIN_CONFIG_ENV_NAME = 'THORIN_CONFIG';
if (typeof process.env.SCONFIG_URL === 'string') {
  try {
    let tmp = url.parse(process.env.SCONFIG_URL);
    tmp.pathname = '/config';
    SCONFIG_URL = url.format(tmp, false);
  } catch (e) {
    console.warn(`SConfig URL is not valid: ${process.env.SCONFIG_URL}`);
  }
}
...
class ThorinConfig {
   ...
   * */
  static secret(app, config, opt, onLoad) {
    if (!opt) opt = {};
    ...
    try {
      ...
      if (files.length === 0) return onLoad();
      files.forEach((fpath) => {
  ...
  static disk(app, config, opt, onLoad) {
      ...
      if (path.extname(configPath) === ".json") {
        calls.push((done) => {
          fs.readFile(configPath, {encoding: 'utf8'}, (err, diskConfig) => {
            ...
            this.__processConfiguration(app, diskConfig, () => {
      ...
      if (path.extname(configPath) === ".js") {
        calls.push((done) => {
          ...
          if (typeof diskConfig === 'object' && diskConfig) {
            return this.__processConfiguration(app, diskConfig, () => {
              extend(true, config, diskConfig);
              done();
            });
          }
    ...
    /* Load config from process.env.THORIN_CONFIG */
    if (process.env[THORIN_CONFIG_ENV_NAME]) {
      calls.push((done) => {
        let d;
        try {
          d = JSON.parse(process.env[THORIN_CONFIG_ENV_NAME]);
        } catch (e) {
          return done();
        }
        this.__processConfiguration(app, d, () => {
    ...
    function parseThorinConfig(type, tconfig, shouldExtend) {
      ...
      switch (type) {
        case 'json':
          res = tconfig;
          if (shouldExtend) {
            self.__processConfiguration(app, res, () => {
