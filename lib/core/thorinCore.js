...
class ThorinCore {
  ...
  * */
  lib(name) {
    ...
    name = name.trim(); return this[libraries][name] || null;
  }
  store(name) {
    if(typeof name !== 'string') return null;
  ...
  * */
  addTransport(transport, name) {
    if(typeof transport === 'string') {
      try {
        transport = require(_normalizeRequire(this.root, transport));
      } catch(e) {
  ...
  * */
  addStore(store, name) {
    if(typeof store === 'string') {
      try {
        store = require(_normalizeRequire(this.root, store));
      } catch(e) {
        if(e.code === 'MODULE_NOT_FOUND') {
  ...
  addPlugin(itemName, alias, opt) {
    ...
    if(typeof itemName === 'string') {
      try {
        item = {
          name: alias || itemName,
          fn: require(_normalizeRequire(this.root, itemName)),
          opt: opt
        };
      } catch(e) {
        if(e.code === 'MODULE_NOT_FOUND' && e.message.indexOf(itemName) !== -1) {
  ...
  * */
  addSanitizer(_items) {
    ...
    items.forEach((itemName) => {
      if(typeof itemName === 'string') {
        try {
          let required = require(_normalizeRequire(this.root, itemName));
  ...
  * */
  addLibrary(a, b) {
    ...
    if(typeof a === 'string' && typeof b === 'undefined') {
      let moduleFn;
      try {
        moduleFn = require(_normalizeRequire(this.root, a));
      } catch(e) {
        if(e.code === 'MODULE_NOT_FOUND') {
  ...
  * */
  initComponents(onDone) {
    ...
    function onComponentInit(name) {
      triggerThorinEvent.call(self, THORIN_EVENTS.INIT, name);
    }
    /* init stores */
    calls.push((done) => {
    ...
    async.series(calls, (e) => {
      delete self[onEventFns][THORIN_EVENTS.INIT];
      onDone(e);
    });
  }
  ...
  setupComponents(onDone) {
    ...
    if(typeof setups === 'string') setups = [setups];
    ...
    async.series(calls, (e) => {
      delete self[onEventFns][THORIN_EVENTS.SETUP];
      onDone(e);
    });

    /* Emit the thorin.EVENT.SETUP for the given component. */
    function onComponentSetup(name) {
      triggerThorinEvent.call(self, THORIN_EVENTS.SETUP, name);
    }
  }
  ...
  * */
  runComponents(onDone) {
    ...
    function onComponentRun(name) {
      triggerThorinEvent.call(self, THORIN_EVENTS.RUN, name);
    }
    ...
    async.series(calls, (e) => {
      delete self[onEventFns][THORIN_EVENTS.RUN];
      onDone(e);
    });
  }
...
function _addPlugin(app, item) {
  ...
  if(pluginType === '[object Object]') {
    ...
    pluginObj = new item.fn(app, item.opt);
  }
...
* */
function triggerThorinEvent(eventName, name) {
