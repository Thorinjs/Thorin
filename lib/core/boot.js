...
global.THORIN_CONFIG = (typeof global.THORIN_CONFIG === 'string' ? global.THORIN_CONFIG : path.basename(process.argv[1]));
module.exports = class ThorinBoot {
  ...
  constructor(app) {
    ...
    this.app = app;
    this.info = null;
    this.configName = global.THORIN_CONFIG; // the key in the .thorin file will always be the node.js file name
  }
  ...
  bootstrap() {
    ...
    if (this.app.app !== 'test' && global.THORIN_AUTOLOAD !== false) {
      ...
      if (isSetup && typeof process.pkg === 'undefined') {
  ...
  get persistFile() {
    ...
    let root = (typeof process.pkg === 'undefined' ? this.info.root : process.cwd());
    persistFile = path.normalize(root + '/' + this.info.persistFile);
    return persistFile;
  }
