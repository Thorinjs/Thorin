...
const info = {
  docker: false,    // This will be set to true if we are inside a docker container. The check is done in /proc/1/cgroup (only works on linux)
  modulePaths: [],
  version: null,    // the app's version, defaults to the one found under package.json
  root: typeof global.THORIN_ROOT === 'string' ? global.THORIN_ROOT : process.cwd(), // the current working dir.
  id: null,
...
(function () {
  ...
  try {
    let env = fs.readFileSync(info.root + '/.env', {encoding: 'utf8'});
    setValues(env);
  } catch (e) {
    // Try and set from root/config/.env
    try {
      let env = fs.readFileSync(info.root + '/config/.env', {encoding: 'utf8'});
      setValues(env);
    } catch (e) {
    }
  }
...
class ThorinApp extends ThorinCore {
  ...
  /*
   * Cleans up the process.env variables to avoid leaking any kind of env data.
   * */
  cleanEnv() {
  ...
  run(onDone) {
    ...
    /* Check if we have a --config={configFilePath} that will override all previously loaded configs */
    let cmdConfigPath = this.argv('config');
    if (cmdConfigPath) {
      this.addConfig('disk', {
        path: cmdConfigPath,
        absolute: true
    ...
    /* Next, check if we have any modules to load. */
    calls.push((done) => {
      this.downloadModules(done);
    });
