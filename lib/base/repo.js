...
module.exports = function init(info) {
  ...
  class BaseRepo extends BaseConfig {
    ...
    async setupComponents() {
      ...
      if (!setups && global.THORIN_SETUP !== true) { // nothing to setup
      ...
      for (let i = 0, len = setups.length; i < len; i++) {
        if (!setups[i]) continue;
        let k = setups[i].split('.')[0];
        setupKinds.push(k);
      }
      let isAllSetup = setups === 'all' || global.THORIN_SETUP === true;
    ...
    #setupComponent = async (obj, id) => {
      if (typeof obj.setup === 'function') {
        try {
          ...
          console.log(`Thorin.setup: failed on [${id}]`);
          throw e;
        }
      }
    ...
    addSanitizer(items) {
      ...
      function add(s, opt = {}) {
        ...
        let sObj = new s(this, opt.name);
        self.#sanitizer[opt.name.toUpperCase()] = sObj;
