...
module.exports = function init(info) {
  ...
  class ThorinApp extends BaseRepo {
    ...
    /**
     * This functionality is used to build error namespaces, that can be used throughout
     * the application with a unique error id that can ultimately be translatable.
     * Usage:
     * const { createErrors, buildError } = thorin.errorBuilder('errorNamespace')
     * const ERROR = createErrors({
     *   NOT_FOUND: buildError('The specified resource does not exist', 404),
     *   READ_USER: buildError('DATA.USER_INVALID', 'An error occurred while reading user')
     * });
     *
     * // later in the application
     * throw thorin.error(ERROR.READ_USER, {someAdditional: 'data'});
     *  // or
     * throw thorin.error('DATA.CUSTOM_CODE', ERROR.NOT_FOUND);
     * // or
     * throw thorin.error('DATA.AUTH', ERROR.READ_USER, 401); // this overrides the error code.
     * */
    errorBuilder(ns) {
      if (typeof ns !== 'string' || !ns) {
        throw new Error('thorin.errorBuilder: requires a string');
      }
      if (errorBuilders[ns]) {
        return errorBuilders[ns];
      }
      const b = {
        ns,
        def: {},
        /**
         * Helper function to build up error information.
         * @Args
         * - errorCode - the static error code to use.
         * - msg - the error message to use
         * - statusCodeOrData? - the status code to use, or the error data.
         * Usage:
         *  buildError(msg)
         *  buildError(code, msg)
         *  buildError(code,msg,statusCode)
         *  buildError(msg,statusCode)
         * */
        buildError: function(a, b, c, d) {
          let code,
            msg,
            status,
            data;
          if (typeof a === 'string' && arguments.length === 1) {
            // buildError(msg)
            msg = a;
          } else if (typeof a === 'string' && typeof b === 'string' && arguments.length === 2) {
            // buildError(code,msg)
            code = a;
            msg = b;
          } else if (typeof a === 'string' && typeof b === 'string' && arguments.length > 2) {
            // buildError(code, msg, statusCodeOrData)
            code = a;
            msg = b;
            if (typeof c === 'number') {
              status = c;
          ...
          } else if (typeof a === 'string' && (typeof b === 'object') || (typeof b === 'number')) {
            // buildError(msg, statusCodeOrData)
            msg = a;
            if (typeof b === 'number') {
              status = b;
              // buildError(msg,statusCode,data)
        ...
        /**
         * Maps together the errors into a single namespace
         * */
        createErrors: function(obj) {
          Object.keys(obj).forEach((errId) => {
            let eObj = obj[errId];
            eObj.eid = `${b.ns}.${errId}`.toLowerCase();
            b.def[errId] = eObj;
          });
          return b.def;
        }
    ...
    /**
     * Retrieves a normalized version of all registered error builders.
     * */
    getErrorBuilders() {
      const res = {};
      Object.keys(errorBuilders).forEach(ns => {
        res[ns] = errorBuilders[ns].def;
      });
      return res;
    }
     ...
     * Ways to call:
     *   thorin.error(preparedError=obj)
     *   thorin.error(code=string, preparedError=obj)
     *   thorin.error(preparedError=obj,statusCode=number)
     *   thorin.error(preparedError=obj,dataOrError=object)
     *   thorin.error(preparedError=obj,statusCode=number,dataOrError=object)
    ...
    error(a, b, c, d) {
      let eid,
        err,
        pcode;
      // check code, preparedError
      if (typeof a === 'string' && typeof b === 'object' && b && b.eid) {
        pcode = a;
        a = b;
        b = c;
        c = d;
      ...
      // check preparedError
      if (typeof a === 'object' && a && a.eid) {
        err = new TError(pcode || a.code, a.message);
        err.id = a.eid;
        if (a.data) err.data = a.data;
        if (a.status) err.statusCode = a.status;
        if (typeof b === 'number') {    // error(p, statusCode)
          err.statusCode = b;
          if (typeof c === 'object' && c) { // error(p, statusCode,data)
        ...
        } else if (typeof b === 'object' && b) { // error(p, data)
