...
module.exports.cloneOpt = function cloneOpt(opt, parentOpt = {}) {
  let newOpt = JSON.parse(JSON.stringify(parentOpt));
  try {
    newOpt = {
      ...newOpt,
      ...opt
    };
    if (typeof opt !== 'object' || !opt) return newOpt;
    ...
    return JSON.parse(JSON.stringify(newOpt));
  } catch (e) {
    return newOpt;
  }
}
