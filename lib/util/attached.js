...
module.exports = function AttachUtilities(app) {
  ...
  /*
  * Utility function that uses thorin.root + path for requiring.
  * */
  app.require = function RequireFromRoot(_path) {
    if(typeof _path !== 'string') throw new Error('thorin.require(path) requires a string path');
    let tmp = path.normalize(thorin.root + '/node_modules/' + _path);
    return require(tmp);
  };
