...
module.exports = function(thorin) {
  ...
  function createFetcher(url, opt, _name) {
    ...
    function doDispatch(action, _payload, _options, done) {
      let bodyPayload = {
        type: action,
        payload: {}
      };
      ...
      if(typeof _options === 'function') done = _options;
      try {
  ...
  *
  * */
  fetcherObj.fetch = function DoFetch(url, opt) {
  ...
  function doFetch(url, opt, done) {
    ...
    nodeFetch(url, opt).then((res) => {
      ...
      return res.text().then((text) => {
        if(isDone) return; isDone = true;
        let tErr = thorin.error('FETCH.ERROR', 'Failed to execute fetch', statusCode);
        tErr.data = text;
        isDone = true;
        done(tErr);
      });
