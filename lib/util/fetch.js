...
module.exports = function (thorin) {
  ...
  try {
    thorinVersion = thorin.version.split('.');
    ...
    function doDispatch(action, _payload, _options) {
      ...
      return nodeFetch(url, fetchOptions)
        .then((res) => {
          ...
          err.ns = code.indexOf('.') === -1 ? 'FETCH' : code.split('.')[0];
          if (errData.data) err.data = errData.data;
          throw err;
        }).catch((e) => {
          if (e && e.ns) return Promise.reject(e);
          let msg = '',
            status = 400,
            code = 'FETCH.';
          if (e) {
      ...
      if (contentType && contentType.indexOf('/json') !== -1) {
        return res.json().then((err) => {
          ...
          let msg = errData.message || 'Failed to execute fetch',
            ...
            code = errData.code || 'FETCH.ERROR';
          let tErr = thorin.error(code, msg, status, (typeof err === 'object' && err && !err.error ? err : null));
          tErr.ns = errData.ns || 'FETCH';
