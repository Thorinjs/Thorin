...
module.exports = function init(app, info) {
  ...
  /**
   * Checks if we need to trigger a LOG_ERROR event
   * */
  function checkErrorTrigger(m) {
    if (!(m instanceof Error)) return;
    let shouldTriggerEvent = false;
    if (m instanceof TypeError) {
      shouldTriggerEvent = true;
    } else if (m instanceof RangeError) {
      shouldTriggerEvent = true;
    } else if (m instanceof ReferenceError) {
      shouldTriggerEvent = true;
    } else if (m instanceof SyntaxError) {
      shouldTriggerEvent = true;
    }
    if (shouldTriggerEvent) {
      app._triggerThorinEvent(events.EVENT.LOG_ERROR, 'thorin.core', m);
    }
  }
  ...
  function output(level, name, newArgs) {
    ...
    for (let i = 0, len = newArgs.length; i < len; i++) {
      if (typeof newArgs[i] === 'undefined' || newArgs[i] === null) continue;
      cleanArgs.push(newArgs[i]);
    }
    newArgs = cleanArgs;
    let msg = newArgs.splice(0, 1)[0] || '';
    checkErrorTrigger(msg);
    let errArgs = [];
