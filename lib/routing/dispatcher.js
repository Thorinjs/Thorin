...
module.exports = function init(thorin) {
  ...
  class ThorinDispatcher extends EventEmitter {
    ...
    * */
    addTemplate(actionObj) {
      ...
      attachTemplate.call(this, actionObj);
      return actionObj;
    }
  ...
  * */
  function attachTemplate(actionObj) {
    ...
    if(actionObj.templates) {
      process.nextTick(() => {
        for(let i=0; i < actionObj.templates.length; i++) {
          actionObj.template(actionObj.templates[i]);
        }
        delete actionObj.templates;
      });
    }
  }
