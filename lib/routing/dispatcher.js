...
module.exports = function init(thorin) {
  ...
  class ThorinDispatcher extends EventEmitter {
    ...
    addAction(actionObj, opt) {
      ...
      if (!(actionObj instanceof thorin.Action) && typeof actionObj.use !== 'function') {
    ...
    dispatch(action, payload, _intentObj, _preserveAuth) {
      ...
      return new Promise((resolve, reject) => {
        let intentObj = new thorin.Intent(action, payload, (wasErr, data) => {
     ...
     * */
    validateIntentFilter(intentObj, validations, onDone) {
      if (typeof validations !== 'object' || validations == null) {
        console.error('Thorin.dispatcher.validateIntentFilter: validation data must be a key-value object');
        return onDone();
      }
      let calls = [],
        inputData = intentObj.rawFilter || {};
      Object.keys(validations).forEach((keyName) => {
        let validatorObj = validations[keyName];
        if (!(validatorObj instanceof IntentValidator)) {
          console.error('Thorin.dispatcher.validateIntentFilter: please use dispatcher.validate() with your action input() field ' + keyName + ' in action ' + intentObj.action);
          return;
        }
        calls.push((done) => {
          validatorObj.run(keyName, inputData[keyName], (e, keyValue) => {
