...
module.exports = function init(app) {
  ...
  class ThorinMiddleware {
    ...
    async _runStack(intentObj, opt, onDone) {
      ...
      try {
        ...
        for (let i = 0, len = this.#filter.length; i < len; i++) {
          ...
          await this.#runStackItem(item, intentObj, opt, HANDLER_TYPE.FILTER);
        }
        /* Next, go for validates */
        for (let i = 0, len = this.#validate.length; i < len; i++) {
          if (intentObj.completed) break;
          let item = this.#validate[i];
          await this.#runStackItem(item, intentObj, opt, HANDLER_TYPE.VALIDATE);
        }
        ...
        for (let i = 0, len = stack.length; i < len; i++) {
          if (intentObj.completed) break;
          let item = stack[i];
          await this.#runStackItem(item, intentObj, opt, HANDLER_TYPE.USE);
        }
      } catch (e) {
        err = e;
      }
     ...
     * */
    #runStackItem = async (item, intentObj, opt = {}, type) => {
      ...
      if (type === HANDLER_TYPE.USE) {
        ...
        if (typeof item.condition === 'function') {
          ...
          try {
            ...
            console.log('thorin.midleware._runStack: use() function threw an error in middleware condition ' + this.name);
            console.log(e, item);
            return;
          }
        ...
        if (item.name) {
          ...
          await mObj._runStack(intentObj, commonUtil.cloneOpt(item.opt, opt));
          return;
        }
        ...
        if (item.fn) {
          return new Promise((resolve, reject) => {
            ...
            try {
              let p = item.fn(intentObj, doneWrap, commonUtil.cloneOpt(item.opt, opt));
              if (commonUtil.isPromise(p)) {
                p.catch(doneWrap);
              }
            } catch (e) {
              console.log('thorin.middleware._runStack: use() function threw an error in authorization ' + this.name);
              console.log(item);
