...
module.exports = function(thorin) {
      ...
      this.stack.forEach((item) => {
        ...
        if (item.type === HANDLER_TYPE.VALIDATE) {
          calls.push((done) => {
            ...
            runHandler.call(this, 'before', HANDLER_TYPE.VALIDATE, intentObj, null, item.value);
            thorin.dispatcher.validateIntent(intentObj, item.value, (e) => {
              runHandler.call(this, 'after', HANDLER_TYPE.VALIDATE, intentObj, null, item.value, e);
              done(e);
            });
          });
          return;
        ...
        if (item.type === HANDLER_TYPE.MIDDLEWARE) {
          ...
          if (!middlewareObj) {
            console.error('Thorin.action.runStack: dispatcher does not have a middleware called ' + item.name + ' for action ' + this.name);
            return;
          }
          calls.push((done) => {
            if (intentObj.completed) return done();  // skip, we completed.
            runHandler.call(this, 'before', HANDLER_TYPE.MIDDLEWARE, intentObj, item.name);
            middlewareObj.runStack(intentObj, item.opt, (e) => {
              runHandler.call(this, 'after', HANDLER_TYPE.MIDDLEWARE, intentObj, item.name, e);
              done(e);
            });
          });
          return;
        ...
        if (item.type === HANDLER_TYPE.AUTHORIZE) {
          ...
          if (!authObj) {
            console.error('Thorin.action.runStack: dispatcher does not have an authorization called ' + item.name + ' for action ' + this.name);
            return;
          }
          calls.push((done) => {
            if (intentObj.completed) return done();  // skip, we completed.
            runHandler.call(this, 'before', HANDLER_TYPE.AUTHORIZE, intentObj, item.name);
            authObj.runStack(intentObj, item.opt, (e) => {
              runHandler.call(this, 'after', HANDLER_TYPE.AUTHORIZE, intentObj, item.name, e);
              done(e);
            });
          });
          return;
        ...
        if (item.type === HANDLER_TYPE.USE) {
          ...
          calls.push((done) => {
            ...
            try {
              ...
              console.error('Thorin.action.runStack: use('+item.fn.name+') function threw an error in action '+ this.name);
              console.error(e);
  ...
  /* Runs the given event handler. */
  function runHandler(handlerType, eventName, intentObj, subName, _arg1, _arg2) {
    ...
    for (let i = 0; i < this.events[handlerType][eventName].length; i++) {
      ...
      if (typeof subName === 'string' && item.name !== subName) continue;
      try {
  ...
  function addHandler(handlerType, type, a, b) {
    ...
    if (!hasHandlerType(type)) {
      console.error('Thorin.action: ' + handlerType + ' type ' + type + " of action " + this.name + ' is not recognized.');
      return this;
    }
    var item;
    if (type === HANDLER_TYPE.MIDDLEWARE || type === HANDLER_TYPE.AUTHORIZE) {  // we have type, name, fn
      ...
      if (typeof a !== 'function') {
        console.error('Thorin.action: ' + handlerType + ' ' + type + ' must have syntax: ' + handlerType + '(type, fn) in action ' + this.name);
        return this;
      }
      item = {
        fn: a
  ...
  /* Checks that the given type exists. */
  function hasHandlerType(type) {
    var items = Object.keys(HANDLER_TYPE),
      isFound = false;
    Object.keys(items).forEach((idx) => {
      let key = items[idx];
      if (HANDLER_TYPE[key] === type) {
        isFound = true;
      }
    });
    return isFound;
